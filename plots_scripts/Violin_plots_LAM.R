...
data_wide = read.delim("LAM.txt", header = T, as.is = T, sep = ",")
...
countX$ypos = c(0, 0, 0, 0, 0, 0, 0, 0, 0)
countX$Gene <- factor(countX$Gene, levels = countX$Gene <- factor(countX$Gene, levels = c("Minar2.cLAD","Grxcr2.cLAD","X4930426D05Rik.cLAD","Hspa9.iLAD","Nars.iLAD","Atp5a1.iLAD", "Pkm.chr9", "P4hb.chr11", "Hsp90ab1.chr17")))

countX = as.data.frame(countX)


setwd("/Users/anamota/Desktop")
pdf("LAM.pdf", height=4, width=6) 

data_wide %>% 
  gather(key="Lamina", value="FRET_efficiency") %>%
  mutate( Lamina=factor(Lamina,levels=c("Minar2.cLAD","Grxcr2.cLAD","X4930426D05Rik.cLAD","Hspa9.iLAD","Nars.iLAD","Atp5a1.iLAD", "Pkm.chr9", "P4hb.chr11", "Hsp90ab1.chr17"))) %>%
  ggplot( aes(x=Lamina, y=FRET_efficiency, color=factor(Lamina))) +
